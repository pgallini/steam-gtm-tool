    ...
    *,
    ...
    for rank, result in enumerate(search_steam_by_tag_ids(tag_ids, max_results=max_results), start=rank_offset):
        ...
        try:
            ...
            candidate = add_discovered_candidate(
                run,
                appid,
                rank,
                {
...
def enrich_run(run_id: str) -> dict[str, Any]:
    ...
    if seed_appid:
        ...
        seen_appids.add(as_int(seed_appid))
        games_processed += 1
        addRunProgressEvent(
            run_id,
            'enrichment',
    ...
    seen_appids.update(as_int(candidate.get('steam_appid')) for candidate in candidates if candidate.get('steam_appid'))
...
def persist_scoring_batches(score_records: list[dict[str, Any]], candidate_records: list[dict[str, Any]]) -> None:
    if score_records:
...
def score_run(run_id: str) -> dict[str, Any]:
    ...
    appids = sorted({as_int(candidate.get('steam_appid')) for candidate in candidates if candidate.get('steam_appid')})
    apps_by_appid: dict[int, dict[str, Any]] = {}
    for start in range(0, len(appids), 500):
        appid_batch = appids[start:start + 500]
        app_filters = {'appid': f'in.({",".join(str(appid) for appid in appid_batch)})'}
        for app in client.select('steam_apps', '*', app_filters):
            apps_by_appid[as_int(app.get('appid'))] = app

    scored: list[dict[str, Any]] = []
    score_records: list[dict[str, Any]] = []
    candidate_records: list[dict[str, Any]] = []
    filtered_count = 0
    progress_interval = max(1, as_int(os.getenv('STEAM_GTM_SCORING_PROGRESS_EVENT_INTERVAL'), 10))

    def record_progress(index: int, candidate: dict[str, Any], details: dict[str, Any]) -> None:
        if index % progress_interval != 0 and index != len(candidates):
            return
        addRunProgressEvent(
            run_id,
            'scoring',
            ...
            details={'candidate_id': candidate['id'], 'title': candidate.get('title'), **details},
        )

    for index, candidate in enumerate(candidates, start=1):
        if candidate.get('is_user_excluded'):
            candidate_records.append({**candidate, 'pipeline_status': 'excluded_by_user'})
            record_progress(index, candidate, {'status': 'excluded_by_user'})
            continue
        appid = candidate.get('steam_appid')
        app = apps_by_appid.get(as_int(appid)) if appid else None
        ...
        if not keep:
            ...
            record_progress(index, candidate, {'status': 'filtered_out', 'reason': filter_reason})
            continue
        ...
        score_records.append(score_payload)
        ...
        record_progress(index, candidate, {'fit_score': round(fit_score, 4)})
        scored.append(
            {
                'candidate': candidate,
                'score': fit_score,
    ...
    for rank, item in enumerate(ranked_items, start=1):
        ...
        candidate_records.append({**candidate, **updates})

    persist_scoring_batches(score_records, candidate_records)
...
""".strip()
    ...
    for start in range(0, len(rows), batch_size):
        ...
        if progress_callback:
            progress_callback(min(start + len(batch_rows), len(rows)), len(rows))
    return results


def selected_classification_rows(run_id: str) -> list[dict[str, Any]]:
...
def classify_run(rule_id: str, run_id: str) -> dict[str, Any]:
    ...
    rows = selected_classification_rows(run_id)
    ...
    if llm_is_configured() and rows:
        try:
            llm_results = llm_classify_rows(
                rows,
                model,
                progress_callback=lambda processed, total: addRunProgressEvent(
                    run_id,
                    'classification',
                    processed,
                    total,
